...
exports.GaService = (() => {
    ...
    let _classThis;
    var GaService = _classThis = class {
        constructor() {
            this.ga('create', environment_1.environment['matGaId'], 'auto', 'mat');
            this.ga('create', environment_1.environment['ngGaId'], 'auto', 'ng');
        }
        locationChanged(url) {
            this.sendPage(url);
        }
        sendPage(url) {
            ...
            if (url === this.previousUrl || !environment_1.environment.production) {
                return;
            }
        ...
        ga(...args) {
            const gaFn = window['ga'];
            if (gaFn) {
                gaFn(...args);
            }
        }
    };
    __setFunctionName(_classThis, "GaService");
    (() => {
        __esDecorate(null, _classDescriptor = { value: _classThis }, _classDecorators, { kind: "class", name: _classThis.name }, null, _classExtraInitializers);
        GaService = _classThis = _classDescriptor.value;
        __runInitializers(_classThis, _classExtraInitializers);
    })();
    return GaService = _classThis;
})();
